...
type Users struct {
	DelFlag     int       `orm:"column(del_flag)"`
	Id          int       `orm:"column(id);pk"`
	Password    string    `orm:"column(password);size(255)"`
	Power       int       `orm:"column(power);null"`
	RegisteDate time.Time `orm:"column(registe_date);type(datetime);null"`
	Score       int       `orm:"column(score);null"`
	Username    string    `orm:"column(username);size(120)"`
}
